...
pub fn fetch_serie_a_team_analysis() -> AnalysisFetch {
    ...
    let teams = match fetch_league_teams(client, 55) {
        Ok(teams) => teams,
        Err(err) => {
            errors.push(format!("serie a teams fetch failed: {err}"));
            Vec::new()
        }
    };
...
pub fn fetch_ligue1_team_analysis() -> AnalysisFetch {
    ...
    let teams = match fetch_league_teams(client, 53) {
        Ok(teams) => teams,
        Err(err) => {
            errors.push(format!("ligue 1 teams fetch failed: {err}"));
            Vec::new()
        }
    };
