...
pub fn default_db_path() -> Option<PathBuf> {
    app_cache_dir().map(|dir| dir.join("premier_league_matches.sqlite"))
}
...
pub fn ingest_all_premier_league_matches(
    ...
    let seasons = fetch_available_seasons(client)?;
    if seasons.is_empty() {
        return Err(anyhow!("no seasons available from FotMob league endpoint"));
    }

    let started_at = Utc::now().to_rfc3339();
    conn.execute(
        "INSERT INTO ingest_runs(started_at, finished_at, seasons_total, seasons_succeeded, matches_upserted, errors_json)
         VALUES (?1, NULL, ?2, 0, 0, '[]')",
        params![started_at, seasons.len() as i64],
    )
    ...
    for season in &seasons {
        match fetch_season_matches(client, season) {
            Ok(rows) => {
    ...
    let latest_utc_time = conn
        .query_row(
            ...
            params![PREMIER_LEAGUE_ID as i64],
...
pub fn load_finished_premier_league_matches(conn: &Connection) -> Result<Vec<StoredMatch>> {
    ...
    let rows = stmt
        .query_map(params![PREMIER_LEAGUE_ID as i64], |row| {
            Ok(StoredMatch {
...
fn fetch_available_seasons(client: &reqwest::blocking::Client) -> Result<Vec<String>> {
    let value = fetch_league_payload(client, None)?;
...
fn fetch_season_matches(
    ...
    let value = fetch_league_payload(client, Some(season))?;
    let matches = value
        .get("fixtures")
        .and_then(|v| v.get("allMatches"))
        .and_then(|v| v.as_array())
        .ok_or_else(|| anyhow!("missing fixtures.allMatches for season {season}"))?;

    let mut out = Vec::with_capacity(matches.len());
    for m in matches {
        if let Some(row) = parse_stored_match(m, season) {
            out.push(row);
        }
    }
    Ok(out)
...
fn fetch_league_payload(client: &reqwest::blocking::Client, season: Option<&str>) -> Result<Value> {
    let url = league_fixtures_url(season);
...
fn league_fixtures_url(season: Option<&str>) -> String {
    let mut url =
        format!("{FOTMOB_LEAGUE_URL}?id={PREMIER_LEAGUE_ID}&tab=fixtures&type=league&timeZone=UTC");
...
fn parse_stored_match(v: &Value, season: &str) -> Option<StoredMatch> {
    let match_id = as_u64_any(v.get("id")?)?;
    let league_id = as_u32_any(v.get("leagueId").unwrap_or(&Value::from(PREMIER_LEAGUE_ID)))
        .unwrap_or(PREMIER_LEAGUE_ID);
    let round = v
        .get("round")
        .and_then(as_i64_any)
